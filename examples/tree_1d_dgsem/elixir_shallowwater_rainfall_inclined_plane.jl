...
equations = ShallowWaterEquations1D(gravity = 9.81, threshold_limiter = 1e-10)
...
function boundary_condition_outflow(u_inner, orientation::Integer, direction, x, t,
                                    ...
                                    equations::ShallowWaterEquations1D)
    if u_inner[1] < 1e-8
        return boundary_condition_slip_wall(u_inner, orientation, direction, x, t,
                                            surface_flux_functions, equations)
    else
        surface_flux_function, nonconservative_flux_function = surface_flux_functions
        # Impulse and bottom from inside, height from external state
        u_outer = SVector(1e-8, u_inner[2], u_inner[3])

        # calculate the boundary flux
        flux = surface_flux_function(u_inner, u_outer, orientation, equations)
        noncons_flux = nonconservative_flux_function(u_inner, u_outer, orientation,
                                                     equations)
...
time_series = TimeSeriesCallback(semi, [(2000.0)])
# The time series callback can be used to evaluate the discharge at the right boundary (x = 2000).

save_solution = SaveSolutionCallback(interval = 10000,
                                     save_initial_solution = true,
                                     save_final_solution = true)

callbacks = CallbackSet(summary_callback, analysis_callback, alive_callback, time_series,
                        save_solution)
