...
style throughout TrixiShallowWater.jl, even at the expense that not everyone can be happy with every
...
## Conventions
The following lists a few coding conventions for TrixiShallowWater.jl. Note that in addition to these
...
source code of TrixiShallowWater.jl, which will also enforce *some* of the [Conventions](@ref) listed
...
[`.JuliaFormatter.toml`](https://github.com/trixi-framework/TrixiShallowWater.jl/blob/main/.JuliaFormatter.toml)
file that holds the configuration options we use for JuliaFormatter.jl.

Note that we expect all contributions to TrixiShallowWater.jl to be formatted with JuliaFormatter.jl
...
```
from inside the TrixiShallowWater.jl repository. For convenience, there is also a script you can
...
```shell
utils/trixisw-format-file.jl path/to/file.jl
```
...
runs over the whole TrixiShallowWater.jl directory.
